...
**Ngày:** 2026-06-16
**Scope:**
- FR-01 Pairing (ghép nối thiết bị con với tài khoản phụ huynh)
- FR-02 Anti-Tamper (chống trẻ lách / vô hiệu hóa kiểm soát)

---

## 1. Executive Summary

Tài liệu đặc tả User Story cho **FR-01: Pairing** — luồng khởi tạo của giải pháp **Parental Control** trên mobile app. Đây là bước đầu tiên bắt buộc: phụ huynh cài App Parents, xác thực, tạo mã ghép nối; người lớn cài App Kid lên máy con, cấp đầy đủ quyền hệ thống + chứng chỉ chặn traffic, rồi quét QR / nhập mã để liên kết máy con vào tài khoản phụ huynh. Pairing thành công là điều kiện tiên quyết để mọi tính năng kiểm soát (FR-05 lọc web, FR-07 quản lý app/thời gian, FR-09 lịch...) hoạt động.

**Mô hình hoạt động (giả định nền tảng — cần PM xác nhận):**

| Hạng mục | Quyết định |
|---|---|
| Account model con | App Kid không có tài khoản riêng — là **agent** của tài khoản phụ huynh |
| Xác thực | SĐT + OTP qua SMS (login = signup, không tách bước đăng ký) |
| Phương thức ghép nối | **Ưu tiên Quét QR**; dự phòng: nhập SĐT phụ huynh → OTP gửi về **máy phụ huynh** |
| Kiểm soát Android | CA Certificate + VPN nội bộ + Accessibility + Device Admin |
| Kiểm soát iOS | MDM Profile (ràng buộc supervision — xem Q&A) |

**Actors:**
- **Phụ huynh** — người dùng App Parents.
- **Người cài máy con** — thường là phụ huynh, thao tác trực tiếp trên máy con khi cài App Kid.
- **App Kid** — agent chạy trên máy con, không login riêng.
- **Hệ thống** — backend server Parental Control.

⚠ **Sensitive data. Legal check required** — luồng này xử lý SĐT, OTP, và cài CA/MDM can thiệp traffic thiết bị. Cần pháp chế rà soát (đồng thuận người dùng, thông báo giám sát).

**Về FR-02 (Anti-Tamper):** nhóm tính năng chống trẻ lách/vô hiệu hóa kiểm soát — gỡ App Kid, đổi giờ hệ thống, tắt VPN/MDM/CA, tắt Accessibility/force-stop, safe mode, factory reset.

> **Nguyên tắc cốt lõi (cần PM/Tech Lead chốt — xem OQ-08):** với mô hình **user tự cài** (Android Device Admin / iOS unsupervised), phần lớn hành vi lách **KHÔNG ngăn cứng được** → App Kid chỉ **phát hiện + cảnh báo phụ huynh + tự phục hồi + áp fail-safe**. Muốn **ngăn chặn cứng** phải nâng lên **Device Owner (Android)** + **Supervised/ABM (iOS)** — đổi lại setup phức tạp, cần kiểm soát máy từ lúc mua. Mỗi US FR-02 dưới đây ghi rõ ranh giới *ngăn cứng* vs *phát hiện+cảnh báo*.
...
| US-06-K | Hoàn tất ghép nối (Quét QR / SĐT + OTP) | Người cài máy con | US-05-K pass, có mã từ US-04-P | Máy con liên kết tài khoản phụ huynh |
